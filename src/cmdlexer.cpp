			...
			size_t tokenlen)
{
	...
	lr->tokens[lr->token_count] = (char*) malloc(tokenlen + 1);
...
void Lexer_AddSubcommandMarkers(struct Lexer* lr, const char left, const char right)
{
	if(lr->subcmdmarker_length >= lr->subcmdmarker_reserved)
	{
		...
		lr->subcmdmarkers = (char**)
		  realloc(lr->subcmdmarkers, sizeof(char*) * lr->subcmdmarker_reserved);
	}

	lr->subcmdmarkers[lr->subcmdmarker_length] = (char*) malloc(2);
...
char* Lexer_ExtractSubcommands(const struct Lexer* lexer, struct LexerResult* lxresult, const char* str, size_t len)
{
	...
	char* parsedstr = (char*) malloc(len + 1);
	...
	subcmdindices = (struct SubCommandIndex*)
	  malloc(sizeof(struct SubCommandIndex) * lxresult->subcmd_reserved);
	memset(subcmdindices, 0, sizeof(struct SubCommandIndex) * lxresult->subcmd_reserved);

	if(!lxresult->subcommands)
	{
	  lxresult->subcommands = (char**)
		  malloc(sizeof(char*) * lxresult->subcmd_reserved);
		memset(subcmdindices, 0, sizeof(struct SubCommandIndex) * lxresult->subcmd_reserved);
	}
	...
	for(; markeridx < lexer->subcmdmarker_length; ++markeridx)
	{
		...
		do
		{
			start = (char*) memchr(&str[idx], left, len - idx);
			if(start)
			{
			  end = (char*) memchr(&str[idx], right, len - idx);
				...
				if(end && end > start)
				{
					if(lxresult->subcmd_length >= lxresult->subcmd_reserved)
					{
						...
						lxresult->subcommands = (char**)
						  realloc(lxresult->subcommands,
									sizeof(char*) * lxresult->subcmd_reserved);
						subcmdindices = (struct SubCommandIndex*)
						  realloc(subcmdindices, sizeof(struct SubCommandIndex) *
					...
					lxresult->subcommands[lxresult->subcmd_length] =
					  (char*) malloc(subcmdlen + 1);
