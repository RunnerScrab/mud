...
void hrt_prioqnode_init(struct hrt_prioqnode* pNode, struct timespec* key, void* data)
{
	pNode->key = *key;
	pNode->data = data;
}

static inline void swap_hrtprioq_node(struct hrt_prioqnode* a, struct hrt_prioqnode* b)
{
...
void hrt_prioq_get_key_at(struct hrt_prioq* pHeap, int idx, struct timespec* out)
{
...
int hrt_prioq_isminheap(struct hrt_prioq* pHeap, size_t idx)
{
	...
	return (rightidx >= len || (rightcmp == -1 && hrt_prioq_isminheap(pHeap, rightidx)))
		|| (leftidx >= len || (leftcmp == -1 && hrt_prioq_isminheap(pHeap, leftidx)));
}
...
static inline void hrt_min_heapify_it(struct hrt_prioq* pHeap, size_t idx)
{
	...
	do
	{
		...
		if(rightidx < arraylen && CmpTs(&arr[smallestidx].key, &arr[rightidx].key) == 1)
		{
...
int hrt_prioq_create(struct hrt_prioq* pHeap, int len)
{
	...
	pHeap->array = (struct hrt_prioqnode*) talloc(sizeof(struct hrt_prioqnode) * pHeap->capacity);
	memset(pHeap->array, 0, sizeof(struct hrt_prioqnode) * pHeap->capacity);
	pHeap->len = 0;
	return (pHeap->array) ? 0 : -1;
}

void hrt_prioq_decreasekey(struct hrt_prioq* pHeap, int idx, struct hrt_prioqnode* pNode)
{
	int i = 0;
...
void hrt_prioq_extract_min_node(struct hrt_prioq* pHeap, struct hrt_prioqnode* pOut)
{
	if(pHeap->len > 0)
	{
		struct hrt_prioqnode* pMin = &(pHeap->array[0]);
...
int hrt_prioq_min_insert(struct hrt_prioq* pHeap, struct timespec* key, void* data)
{
